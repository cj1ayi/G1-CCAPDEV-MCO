...
class PostService {
  ...
  /**
   * Fetches current user ONCE then applies
   * ownership to all posts in the batch.
   */
  private async applyPopulation(
    ...
    return posts
      ...
      .map((post) => {
        const withAuthor =
          this.populateAuthor(post)
        const isOwner = currentUser
          ? currentUser.id ===
              withAuthor.author.id ||
            currentUser.id === withAuthor.authorId
          : false
        return { ...withAuthor, isOwner }
      })
  }
  ...
  async getPostById(
    ...
    try {
      ...
      const data = await response.json()
      const posts = await this.applyPopulation([
        this.mapPost(data),
      ])
      return posts[0] || null
    } catch (err) {
  ...
  validatePostForm(
    ...
    const visibleContent = stripInvisible(
      data.content ?? '',
    )
    if (!visibleContent) {
