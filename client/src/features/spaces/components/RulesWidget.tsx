        ...
        <ol className="list-decimal list-inside space-y-3 text-sm">
          {visibleRules.map((rule, i) => (
            <li key={i} className="pl-2 border-b dark:border-gray-800 pb-2 last:border-0">
              ...
              {rule.description && <p className="text-xs text-gray-500 ml-4">{rule.description}</p>}
            </li>
          ))}
        </ol>
        {rules.length > 3 && (
          <Button variant="ghost" size="sm" fullWidth onClick={() => setShowAll(!showAll)} className="text-primary mt-3">
