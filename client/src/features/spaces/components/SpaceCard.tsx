      ...
      <div className="space-y-4">
        <div className="flex justify-between items-start">
          <div className="relative">
            <div className={cn(
              "size-14 rounded-xl flex items-center justify-center text-white shadow-inner overflow-hidden",
              ...
              {space.iconType === 'image' ? (
                ...
                <span className={cn(
                  "font-black",
                  isMaterialIcon ? "material-symbols-outlined text-3xl" : (space.icon.length > 2 ? "text-lg" : "text-2xl")
                )}>
                  {space.icon}
                </span>
              )}
        ...
        <div>
          <h3 className="text-lg font-bold group-hover:text-primary transition-colors">{space.displayName}</h3>
          <p className="text-gray-500 text-sm leading-relaxed line-clamp-2">{space.description}</p>
        </div>
      </div>
